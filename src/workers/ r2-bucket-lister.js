// Stem descriptions from README files
...
const VALID_BUCKET_NAMES = TRACKS.map(t => t.id);
...
export default {
  async fetch(request, env) {
    ...
    // Get bucket name from path: /hydrogen, /lithium, etc.
    const pathParts = url.pathname.split('/').filter(p => p);
    const bucketName = pathParts[0] || '';
    const fileName = decodeURIComponent(pathParts.slice(1).join('/')); // Decode URL-encoded characters

    // Validate bucket name
    if (bucketName && !VALID_BUCKET_NAMES.includes(bucketName.toLowerCase())) {
      return new Response('Invalid bucket name', {
        status: 404,
    ...
    if (!env.HYDROGEN || !env.LITHIUM || !env.SODIUM || !env.POTASSIUM || !env.RUBIDIUM || !env.CAESIUM || !env.FRANCIUM) {
    ...
    const bucketMap = {
      'hydrogen': env.HYDROGEN,
      'lithium': env.LITHIUM,
      'sodium': env.SODIUM,
      'potassium': env.POTASSIUM,
      'rubidium': env.RUBIDIUM,
      'caesium': env.CAESIUM,
      'francium': env.FRANCIUM
    };

    const bucket = bucketMap[bucketName.toLowerCase()];
    ...
    if (!bucket) {
      ...
      // Show index page with all buckets
      const html = `
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <meta name="description" content="Free audio stems from Software-Entwicklungskit's Everything is Free album. All stems released under CC0 1.0 Universal - no restrictions, no attribution required.">
  ...
  <title>Software-Entwicklungskit - Everything is Free</title>
  <style>
...
<body>
  ...
  <h2>🎵 Tracks</h2>
  <img src="https://artwork.ichbinsoftware.com/Example-Waveform-View.png" alt="Example waveform view of ev3.ichbinsoftware.com" loading="lazy" style="width: 100%; height: auto; display: block;">
  <br/>
  ...
  <footer class="footer">
    <div class="footer-content">
      <ul class="footer-links">
        <li><a href="https://software-entwicklungskit.bandcamp.com/" target="_blank" rel="noopener" aria-label="Visit us on Bandcamp">Bandcamp</a></li>
        <li><a href="https://www.instagram.com/ichbinsoftware/" target="_blank" rel="noopener" aria-label="Follow us on Instagram">Instagram</a></li>
        <li><a href="https://www.youtube.com/@ichbinsoftware" target="_blank" rel="noopener" aria-label="Subscribe on YouTube">YouTube</a></li>
        <li><a href="https://open.spotify.com/artist/6qo7PqvYZvrkIrr8gF2uHL?si=S1I8K4miSFCEki1PMINrXA" target="_blank" rel="noopener" aria-label="Listen on Spotify">Spotify</a></li>
    ...
    // If there's a filename, serve the actual audio file
    if (fileName) {
      try {
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="${bucketName.charAt(0).toUpperCase() + bucketName.slice(1)} audio stems from Software-Entwicklungskit's Everything is Free album. Free download, CC0 licensed.">
  <meta property="og:title" content="Everything is Free - ${bucketName.toUpperCase()} Stems">
  <meta property="og:description" content="Download ${bucketName} audio stems. CC0 licensed.">
  <meta property="og:image" content="https://${bucketName}.ichbinsoftware.com/${bucketName.charAt(0).toUpperCase() + bucketName.slice(1)}-Symbol.png">
  <link rel="preconnect" href="https://unpkg.com">
  <link rel="preconnect" href="https://${bucketName}.ichbinsoftware.com">
  <title>Software-Entwicklungskit - Everything is Free - ${bucketName.toUpperCase()} Stems</title>
...
<body>
  ...
  <div class="main-content">
    ...
    <h2>Everything is Free - ${bucketName.toUpperCase()} Stems</h2>

  ${(() => {
    const track = trackMetadata[bucketName.toLowerCase()];
    if (track) {
      const trackNameCapitalized = track.name;
      const zipFileName = `${track.number}.${trackNameCapitalized}_STEMS.zip`;
      const zipUrl = `https://${bucketName.toLowerCase()}.ichbinsoftware.com/${zipFileName}`;
      return `
  <div class="track-summary">
    <div class="track-image">
      <img src="https://${bucketName.toLowerCase()}.ichbinsoftware.com/${trackNameCapitalized}-Symbol.png" width="100" height="100" alt="${trackNameCapitalized} symbol" loading="eager">
    </div>
    <div class="track-info-table">
      <table>
        ...
        <tbody>
          <tr>
            <td><strong>${trackNameCapitalized}</strong></td>
            <td>${track.bpm}</td>
            <td>${track.key}</td>
            <td>${track.length}</td>
  ...
  ${wavFiles.length === 0 ? `
    <div class="empty">
      <p>No WAV files found in this bucket.</p>
    </div>
  ` : wavFiles.map((obj, index) => {
    const escapedKey = obj.key.replace(/\\/g, '\\\\').replace(/'/g, "\\'");

    // Format file size - R2 uses decimal units (1 MB = 1,000,000 bytes)
    ...
    <div class="file">
      ...
      <div class="controls">
        <button class="btn btn-load" id="load-${index}" data-key="${escapedKey}" aria-label="Load audio player for ${obj.key}">
          📊 Load Player
        </button>
        <button class="btn" id="play-${index}" style="display: none;" disabled aria-label="Play or pause ${obj.key}">
          <span class="loading">Loading...</span>
        </button>
        <span class="time" id="time-${index}" style="display: none;" aria-live="polite">0:00 / 0:00</span>
        <a href="/${bucketName}/${obj.key}" download="${obj.key}" class="btn btn-download" aria-label="Download ${obj.key}">
          ⬇️ Download
        </a>
      </div>
    </div>
    ...
    ${wavFiles.map((obj, index) => `
      document.getElementById('load-${index}').addEventListener('click', function() {
        ...
        ws.on('error', function(error) {
          ...
          placeholder.innerHTML = 'Error loading audio file';
        ...
        ws.on('play', function() {
          ...
          Object.keys(wavesurfers).forEach((key) => {
            if (key !== '${index}' && wavesurfers[key].isPlaying()) {
